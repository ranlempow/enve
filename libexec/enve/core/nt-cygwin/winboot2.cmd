...
rem if exist "%MSYS_BASE%\bin\mintty.exe" set "MSYS_BIN_PREFIX=/bin"
rem if exist "%MSYS_BASE%\usr\bin\mintty.exe" set "MSYS_BIN_PREFIX=/usr/bin"
rem if "%MSYS_BIN_PREFIX%" == "" (
rem     echo "MSYS_BIN_PATH not found in %MSYS_BASE%"
rem     goto :eof
rem )
rem set "MSYS_BIN_PATH=%MSYS_BASE%%MSYS_BIN_PREFIX%"
rem for /f "delims=" %%A in ('%MSYS_BIN_PATH%\uname.exe --operating-system') do set "MSYS_OS=%%A"
rem if "%MSYS_OS%" == "" (
rem     echo "MSYS_OS can not determined"
rem     goto :eof
rem )
rem if not exist "%MSYS_RCFILE%" (
rem     set "MSYS_RCFILE=%MSYS_BASE%\etc\profile"
rem )
...
rem if "%MSYS_OS%" == "Msys" (
rem     set MSYS2_PATH_TYPE=strict
rem     set CYG_SYS_BASHRC=1
rem )

rem if not defined MSYSTEM set MSYSTEM=MSYS
start "" "%MSYS_BASE%\usr\bin\mintty.exe" -c "%USERPROFILE%\.minttyrc" -- "/usr/bin/bash" --rcfile "%MSYS_RCFILE%" -i
goto :eof
